...
def build_epub_panels(container) -> EPUBPanelWidgets:
    ...
    epub_frame.grid(row=2, column=0, columnspan=3, sticky="ew", padx=4, pady=4)
    for column in range(3):
        epub_frame.columnconfigure(column, weight=1 if column == 1 else 0)
    ...
    batch_frame.grid(row=3, column=0, columnspan=3, sticky="ew", padx=4, pady=4)
    for column in range(3):
        batch_frame.columnconfigure(column, weight=1 if column == 1 else 0)
    ...
    ttk.Label(batch_frame, text="批量纸张大小").grid(row=3, column=0, sticky="w", padx=8, pady=6)
    ...
    ttk.Label(batch_frame, text="批量输出目录").grid(row=4, column=0, sticky="w", padx=8, pady=6)
    ...
    ttk.Label(batch_frame, text="输出文件名模板").grid(row=5, column=0, sticky="w", padx=8, pady=6)
